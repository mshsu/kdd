...
def get_stopwords(link: str) -> pd.Series:
    ...
    """
    return pd.read_csv(link, header=None, sep=" ").iloc[:, 0]
...
def vector_from_f(docname, corpus, f_table,
                  words, n, df, idf, avdl, query = False):
    """
...
def vectors_from_f(f_table, query = False):
    """
...
def main():
    ...
    if len(args) < 2:
        ...
        docs = get_filenames('data', samp_size)
        ...
        gt.to_csv("data_ground_truth.csv", header=False, index=False)
